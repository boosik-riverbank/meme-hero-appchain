...
func (m MockBankKeeper) SendCoins(ctx context.Context, fromAddr, toAddr sdk.AccAddress, amt sdk.Coins) error {
	for _, v := range amt {
		...
		if !ok {
			m.balances[fromAddr.String()+v.Denom] = sdk.NewCoin(v.Denom, math.NewInt(0))
			senderBalance, _ = m.balances[fromAddr.String()+v.Denom]
		}
		...
		if !ok {
			m.balances[toAddr.String()+v.Denom] = sdk.NewCoin(v.Denom, math.NewInt(0))
			receiverBalance = m.balances[toAddr.String()+v.Denom]
		}
