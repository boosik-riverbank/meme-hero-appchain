...
func (k Keeper) CreateNewPool(ctx context.Context, msg *types.MsgCreatePool, poolAddress sdk.AccAddress) (uint64, error) {
	...
	actualAmountIn := big.NewInt(0)
	actualNativeIn := big.NewInt(0)
	nativeFee := big.NewInt(0)
	// TODO: move fee to parameters
	createFee := big.NewInt(1000000)
	var err error

	if msg.IsNativeIn {
		_, actualAmountIn, nativeFee, err = k.quoteAmountOut2(amount)

	} else {
		actualNativeIn, _, nativeFee, err = k.quoteAmountIn2(amount)
	}

	k.length++
	poolID := k.length

	_, tokenOffset, nativeOffset, err := k.CalculateConfig()
	config, _ := k.getDefaultBondingCurveConfig()
	if err != nil {
		return 0, err
	}
	...
	pool := types.BondingCurvePool{
		...
		Fee:          "100000",
		TokenOffset:  tokenOffset.String(),
		NativeOffset: nativeOffset.String(),
	}
